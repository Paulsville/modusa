...
public class RatchetingSessionTest extends TestCase {
  ...
  public void testModusaHashInitialisation() throws InvalidKeyException, NoSuchAlgorithmException {
    ...
    AliceSignalProtocolParameters aliceParams = AliceSignalProtocolParameters.newBuilder()
            .setOurBaseKey(aliceBaseKey)
            .setOurIdentityKey(aliceIdentityKey)
            .setTheirIdentityKey(bobIdentityKey)
            .setTheirSignedPreKey(bobSignedPreKey)
    ...
    RatchetingSession.initializeSession(aliceSession, aliceParams);

    byte[] aliceHash1 = aliceSession.getLastFprintHash();
    byte[] aliceHash2 = aliceSession.getFprintHash();

    IdentityKey     bobIdentityKeyPublic   = new IdentityKey(bobIdentityPublic, 0);
    ECPrivateKey    bobIdentityKeyPrivate  = Curve.decodePrivatePoint(bobIdentityPrivate);
    IdentityKeyPair bobIdentityKeyPair         = new IdentityKeyPair(bobIdentityKeyPublic, bobIdentityKeyPrivate);
    bobEphemeralPublicKey  = Curve.decodePoint(bobPublic, 0);
    ECPrivateKey    bobEphemeralPrivateKey = Curve.decodePrivatePoint(bobPrivate);
    ECKeyPair       bobEphemeralKey        = new ECKeyPair(bobEphemeralPublicKey, bobEphemeralPrivateKey);
    ECKeyPair bobSignedPreKeyPair        = new ECKeyPair(Curve.decodePoint(bobSignedPreKeyPublic, 0), Curve.decodePrivatePoint(bobSignedPreKeyPrivate));

    aliceBasePublicKey       = Curve.decodePoint(aliceBasePublic, 0);
    aliceIdentityPublicKey   = new IdentityKey(aliceIdentityPublic, 0);

    BobSignalProtocolParameters bobParams = BobSignalProtocolParameters.newBuilder()
            .setOurIdentityKey(bobIdentityKeyPair)
    ...
    RatchetingSession.initializeSession(bobSession, bobParams);
    byte[] bobHash1 = bobSession.getLastFprintHash();
    byte[] bobHash2 = bobSession.getFprintHash();
